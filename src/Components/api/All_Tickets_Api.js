...
const All_Tickets_Api = async userToken => {
  try {
    const isConnected = await checkInternetConnectivity();
    if (isConnected) {
      // Fetch data from API
      const response = await fetch(
        URL_CONFIG.Url + URL_CONFIG.all_tickets_api,
        {
          method: 'GET',
          headers: {
      ...
      CreateTableFunction('all_tickets_data');
      InsertDataFunction('all_tickets_data', data);
      const dataNew = await GetAllDataFunction('all_tickets_data');
      return dataNew;
    } else {
      const dataNew = await GetAllDataFunction('all_tickets_data');
      return dataNew;
...
export default All_Tickets_Api;
