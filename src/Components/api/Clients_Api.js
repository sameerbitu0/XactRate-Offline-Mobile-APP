...
const Clients_Api = async userToken => {
  try {
    const isConnected = await checkInternetConnectivity();
    if (isConnected) {
      // Fetch data from API
      const response = await fetch(URL_CONFIG.Url + URL_CONFIG.clients_api, {
        method: 'GET',
        headers: {
      ...
      const data = dataResponse.clients;
      CreateTableFunction('clients_data');
      InsertDataFunction('clients_data', data);
      const dataNew = await GetAllDataFunction('clients_data');
      return dataNew;
    } else {
      const dataNew = await GetAllDataFunction('clients_data');
      return dataNew;
