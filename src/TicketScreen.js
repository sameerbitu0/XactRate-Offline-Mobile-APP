...
import Convert_Invoice_Api from './Components/api/Convert_Invoice_Api';
...
const Ticket = () => {
  ...
  const searchClients = clientListData.filter(
    item =>
  ...
  const fetchData = async () => {
    try {
      ...
      const ClientsData = await Clients_Api(userToken);
      const TicketTypeData = await Tickets_Type_Api(userToken);
      const CatalogsData = await Catalogs_Api(userToken);
      const CategoriesData = await Categories_Api(userToken);
      const ProductsData = await Products_Api(userToken);
      const EmployeesData = await Employees_Api(userToken);
      const ServiceAgreementData = await Service_Agreement_Api(userToken);
      const WarrantyPercentageData = await Warranty_Percentage_Api(userToken);
      const ContingencyProductData = await Contingency_Product_Api(userToken);
      setClientListData(ClientsData);
      setTicketTypeData(TicketTypeData);
      setCatalogData(CatalogsData);
      setCategoryData(CategoriesData);
      setProductData(ProductsData);
      setEmployeeData(EmployeesData);
      setServiceAgreementData(ServiceAgreementData);
      setWarranty(WarrantyPercentageData);
      setContingencyProduct(ContingencyProductData);
    } catch (error) {
      return null;
    } finally {
      return null;
    ...
    if (status == 'lock') {
      Alert.alert(
        ...
        [
          ...
          {text: 'Yes', onPress: () => convertToInvoice('locked')},
        ],
      );
      ...
      Alert.alert(
        ...
        [
          ...
          {text: 'Yes', onPress: () => convertToInvoice('suspend')},
        ],
      );
  ...
  const getInvoiceData = async () => {
    ...
    var serviceAgreement = serviceAgreementTotal.toFixed(2);
  ...
  const ticketDecline = async () => {
    ...
    if (isConnected) {
      ...
      alert("User can't decline the ticket without internet.");
    }
  };
  ...
  const selectTicket = async item => {
    ...
    setTicketTypeDescription(item.des);
    setTicketStatus('create_ticket');
    startTdTimePop();
  };
  ...
  const CategoryFunction = async item => {
  ...
  function AddServiceAgreement(item) {
    ...
    var invoiceTotal = parseInt(invoiceTotalAmount) + item.price;
    setInvoiceTotalAmount(invoiceTotal);
      ...
      <>
        ...
        <View style={styles.MainContainerClientList}>
          ...
          {clientListData.length > 0 ? (
            <FlatList
              data={searchQuery.length > 0 ? searchClients : clientListData}
            ...
            <TouchableOpacity
              onPress={() =>
                ...
                isPauseActive == true
                  ...
                  : setSubmitAmountModel(false) + convertToInvoice('closed')
              }
              style={styles.PopUpCancelButton}>
              <LinearGradient
                colors={['#202020', '#606060', '#202020']}
              ...
              onChangeText={text => {
                ...
                if (validatedValue.split('.').length > 2) {
                  validatedValue = validatedValue.slice(
                    0,
                    validatedValue.lastIndexOf('.'),
                  );
                }
                calculateTotalPay(validatedValue);
                setTotalPay(validatedValue);
                setErrorMessage('');
              }}
              style={styles.TotalAmountInput}
            />
        ...
        <ScrollView style={{flex: 1}}>
          <View style={styles.MainContainerProductsData}>
            <TouchableOpacity
              ...
              onPress={() => setDefaultForm(false) + setCustomProduct(true)}>
                  ...
                  <View key={item.employee_id}>
                    <View
                      ...
                      <TextInput
                        ...
                        onChangeText={text => {
                          let validatedValue = text.replace(/[^0-9]/g, '');
                          handleNameChange(validatedValue, index);
                        }}
                        style={{
                          color: 'black',
                          fontSize: 17,
  ...
  const convertToInvoice = async status => {
    try {
      if (!invoiceData.client.id) {
      ...
      const postData = {
        ...
        ticketDate: date,
        ticket_locked: status == 'locked' ? 'locked' : null,
        ticket_suspend: status == 'suspend' ? 'suspend' : null,
        ticket_closed: status == 'closed' ? 'closed' : null,
        signature: signature ? signature : null,
        notes: note ? note : '',
        success: 'true',
      };
      const offlinePostData = {
        first_name: invoiceData.client.fn,
        last_name: invoiceData.client.ln,
        ticket_type_description: TicketDescription
          ? TicketDescription
          : ticketTypeDescription,
        email: invoiceData.client.email,
        phone_no_1: invoiceData.client.phone_no_1,
        ticket_notes: TicketNotes ? TicketNotes : '',
        ticket_id: ticketId,
        ticket_type_id: ticketTypeId,
        status: status == 'closed' ? '2' : '5',
        invoice_status: ticketStatus ? ticketStatus : '',
        client_id: invoiceData.client.id,
        products: invoiceData.products,
        service_agreement: invoiceData.service_agreements,
        timesheetAmount: invoiceData.timeSheet[0].total_amount,
      ...
      const userToken = await AsyncStorage.getItem('userToken');
      const ConvertInvoice = await Convert_Invoice_Api(
        userToken,
        postData,
        offlinePostData,
        index,
      ...
      if (ConvertInvoice) {
        ...
        Alert.alert('Convert invoice successfully.');
      }
    } catch (error) {
